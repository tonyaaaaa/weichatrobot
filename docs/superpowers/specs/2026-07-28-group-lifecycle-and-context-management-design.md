...
1. 基本信息，只读展示机器人、群名称、群备注、导入来源和状态。
2. 知识权限，配置可检索的知识标签。
3. 知识库未命中处理，配置 Web Search 和模型自身知识降级。
4. 上下文策略，配置范围、轮数、超时、Token、摘要和机器人历史。
5. 长期记忆，只读展示群级有效记忆、用户级记忆和待整理候选数量，并提供
   “打开记忆中心”入口。
...
“匹配规则”和“保存前预览”移动到默认折叠的“高级群匹配”。对于
`WorkToolImport` 群显示：
