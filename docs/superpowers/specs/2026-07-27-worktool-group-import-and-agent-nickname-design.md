...
本规格不得与 WorkTool 契约加固合并成一个超大实施计划。书面规格批准后生成以下三份
独立计划：

### 计划 A：WorkTool 官方契约加固

覆盖：

- 删除无官方依据的在线状态字段解析。
- API 与 Worker 共用的出口 IP 级 60 QPM 限流。
- 合法但暂不支持的消息回调成功确认并忽略。
- WorkTool 请求省略 `null` 和空 `atList`。

该计划以
`2026-07-27-worktool-official-contract-hardening-design.md`
为依据，必须先完成，因为群列表和 `type=512` 也需要经过同一个全局限流传输层。

### 计划 B：群列表导入与账号唯一昵称绑定

覆盖：

- WorkTool 群列表客户端和官方分页 DTO。
- 远程群列表、状态匹配、单个及批量导入。
- 用户公司级唯一 WorkTool 昵称绑定。
- 新建群成功后的自动发现。
- 改名成功后的本地协调。
- 群客服数据表和禁用态前端骨架。

本计划不解析 `type=512` 结果，也不允许把未验证昵称配置为有效群客服。

### 计划 C：`type=512` 群成员昵称快照

这是单独的证据驱动计划，覆盖：

1. 在专用测试群发送一条 `type=512` 请求。
2. 从 WorkTool 官方回调或官方查询接口取得真实、脱敏的完整结果样例。
3. 记录结果承载位置、字段类型、空列表、重名、特殊字符和失败结果。
4. 将样例保存为不包含真实人员昵称的合约测试夹具。
5. 先编写失败合约测试，再实现结果 DTO 和解析器。
6. 建立群成员昵称快照和刷新状态。
7. 启用群内客服资格验证和人工通知昵称解析。

证据门：

- 未取得真实结果样例时，计划 C 停在第 2 步并报告外部契约阻塞。
- 不从 `rawMsg`、`successList`、`data` 或其他字段名称进行猜测。
- 不使用模拟出来的“官方样例”让测试通过。
- 真实验收只允许在明确指定的测试机器人和测试群执行。
...
计划 A、B 完成后系统已经可以免手填机器人 ID 导入群并绑定客服唯一昵称；只有计划 C
通过真实证据门后，才开放群内成员验证、群客服选择和可靠的定向 @ 通知。
