...
describe('ConversationAuditView filters', () => {
  it('converts local datetimes to UTC and resets pagination when applying filters', async () => {
    ...
    const wrapper = mount(ConversationAuditView, { props: { api, groupOptionApi } });
    await flushPromises();

    const groupSelector = wrapper.findComponent(GroupProfileSelect);
    ...
    expect(labels).toContain('历史群 · 客服机器人 · 已归档');
  ...
  it('labels the actual answer source and renders only structured web citations', async () => {
    ...
    const wrapper = mount(ConversationAuditView, {
      props: { api, groupOptionApi: { list: vi.fn().mockResolvedValue([]) } }
    });
    await flushPromises();
