...
<template>
  <section class="group-operations-view">
    ...
    <section><h2>新建或调整群</h2><p>先预览，再使用两分钟内有效、且与当前内容绑定的确认令牌执行。</p>
      <label>机器人配置 ID <input v-model="operation.robotConfigId"></label><label>操作 <select v-model="operation.kind"><option value="Create">新建外部群</option><option value="AddMembers">添加成员</option><option value="RemoveMembers">移除成员</option><option value="Rename">改群名</option><option value="UpdateAnnouncement">更新群公告</option></select></label>
      <label>群名称（如已设置群备注，请填备注名）<input v-model="operation.groupIdentifier"></label><label>成员显示名（每行一个）<textarea :value="operation.memberIds.join('\n')" @input="operation.memberIds = ($event.target as HTMLTextAreaElement).value.split('\n').map(item => item.trim()).filter(Boolean)" /></label><label>新值 <textarea v-model="operation.value" /></label>
      <button type="button" @click="preview">预览操作</button><button type="button" :disabled="!confirmationToken" @click="execute">确认执行</button>
    </section>
    <p aria-live="polite">{{ notice }}</p><section><h2>命令状态</h2><button type="button" @click="loadAudit">刷新</button><ul><li v-for="item in audit" :key="`${item.createdAtUtc}-${item.operation}`">{{ item.createdAtUtc }} · 指令 {{ item.workToolCommandNumber }} · {{ item.operation }} · {{ item.status }} {{ item.result }}</li></ul></section>
  </section>
</template>
