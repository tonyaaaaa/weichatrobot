...
export interface GroupOperation { robotConfigId: string; kind: 'Create' | 'AddMembers' | 'RemoveMembers' | 'Rename' | 'UpdateAnnouncement' | 'UpdateRemark'; groupIdentifier: string; memberIds: string[]; value?: string; }
export interface WorkToolOperationsApi {
  ...
  listOperations(): Promise<{ id: string; operation: string; status: string; result?: string; createdAtUtc: string }[]>;
}
export const workToolOperationsApi: WorkToolOperationsApi = {
  ...
  async listOperations() { return (await apiClient.get<{ id: string; operation: string; status: string; result?: string; createdAtUtc: string }[]>('/api/admin/worktool/group-operations')).data; }
};
