...
public interface IHandoffOrchestrator
{
    ...
    Task HandleDecisionAsync(ConversationProcessingRequest request, GroundedAnswerResult result, CancellationToken token);
}
...
public sealed class HandoffOrchestrator(HandoffService handoffs, IHandoffStore store, HandoffTriggerEvaluator triggers, HandoffTriggerOptions options) : IHandoffOrchestrator
{
    ...
    public async Task HandleDecisionAsync(ConversationProcessingRequest request, GroundedAnswerResult result, CancellationToken token)
    {
